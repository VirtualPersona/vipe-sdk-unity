...
namespace CA
{
    ...
    public class CryptoAvatars
    {
        ...
        {
            const string urlServer = "https://api.testnets.cryptoavatars.io:3000/v1/";
            const string urlServerTest = "http://localhost:3000/v1/";
         ...
         * onLoginResult -> callback to know if loggin was susscesfully
         */
        public IEnumerator UserLoginWithMetamaskToCA(string wallet, string signature, Action<Structs.LoginResponseDto> onLoginResult)
        {
            Structs.LoginMetamaskDto loginRequestDto = new Structs.LoginMetamaskDto();
            loginRequestDto.wallet = wallet;
            loginRequestDto.signature = signature;

            return this.httpService.Post("login", loginRequestDto, (string loginResult) => {
                Structs.LoginResponseDto loginResponse = JsonUtility.FromJson<Structs.LoginResponseDto>(loginResult);
                onLoginResult(loginResponse);
            });
        }
         ...
         */
        public IEnumerator GetUserAvatars(string owner, string pageUrl, System.Action<Structs.NftsArray> onAvatarsResult)
        {
         ...
         * onAvatarsResult -> callback to get Avatars info
         */
        public IEnumerator GetAvatars(string licenseType, string pageUrl, System.Action<Structs.NftsArray> onAvatarsResult)
        {
        ...
        public IEnumerator GetAvatarsByName(string licenseType, string pageUrl, string name, Action<Structs.NftsArray> onAvatarsResult)
        {
        ...
        public IEnumerator GetAvatarsByCollectionName(string collectionName, string pageUrl, Action<Structs.NftsArray> onAvatarsResult)
        {
        ...
        * onImage -> callback to get Avatar thumbnail
        */
        public IEnumerator GetAvatarVRMModel(string urlVrm, Action<GameObject, String> onModelResult)
        {
            urlVrm = urlVrm.Replace("gateway.pinata.cloud", "usercollection.mypinata.cloud");
            yield return this.httpService.Download3DModel(urlVrm, (localPath) =>
            {
                GameObject avatar = ImportVRM(localPath);
                onModelResult(avatar, localPath);
            });
        }    
