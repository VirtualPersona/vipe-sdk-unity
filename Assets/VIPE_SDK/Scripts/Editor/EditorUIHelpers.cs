...
namespace VIPE_SDK
{
    public static class EditorUIHelpers
    {
        ...
        public static void DrawLoadingSpinner(Rect rect, Texture2D loadingTexture, float rotationAngle, float margin)
        {
            Texture2D transparentBackground = new Texture2D(1, 1);

            transparentBackground.SetPixel(0, 0, new Color(0, 0, 0, 0.8f));
            transparentBackground.Apply();

            GUI.DrawTexture(rect, transparentBackground);

            float sideLength = Mathf.Min(rect.width, rect.height) - 2 * margin;

            float centerX = rect.x + rect.width / 2;
            float centerY = rect.y + rect.height / 2;

            Rect squareRect = new Rect(centerX - sideLength / 2, centerY - sideLength / 2, sideLength, sideLength);

            Matrix4x4 matrixBackup = GUI.matrix;
            GUIUtility.RotateAroundPivot(rotationAngle, squareRect.center);
            GUI.DrawTexture(squareRect, loadingTexture);
